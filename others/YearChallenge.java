...
public class YearChallenge {
	...
	private final int rangeLow, rangeHi;
	
	
	private YearChallenge(double[] variables, int r0, int r1){
		this.list       = variables;
	...
	public static void solve(double[] variables, int rangeLow, int rangeHi, long timeLimit){
		YearChallenge solver = new YearChallenge(variables, rangeLow, rangeHi);
		HashMap<Integer, String> solutions = new HashMap<Integer, String>();
		
		int _shuffle = 0;
		double result;
		int closest; //Closest int to result
		long start = System.currentTimeMillis();
		
		while(true){ 
			solver.reset(); //reset certain values for next attempt

			result = solver.group(0);

			if(Double.isNaN(result)) continue;
			
			closest = (int) (Math.abs((result))+ 0.5); //Closest int to the result
			
			if(Math.abs(Math.abs(result) - closest) < 0.001 && closest >= rangeLow && closest <= rangeHi && !solutions.containsKey(closest)){ //Close enough to an int to count
				StringBuilder s = new StringBuilder(solver.answer.size());
				for(char c : solver.answer) s.append(c); 
				if(result < 0) s.insert(0, '-');
				solutions.put(closest, s.toString());
			}
			
			if((_shuffle++ & 63) == 0) //Shuffle numbers around every 32 times
				solver.shuffle();
						
			//Occasionally check to see if we passed the time limit
			if((_shuffle & 524287) == 0 && (System.currentTimeMillis() - start)/1000 > timeLimit) 
				break;
        ...
        System.out.printf("\n\nWe found solutions to %d numbers out of %d. \n",solutions.size(), (rangeHi - rangeLow + 1)); 
		System.out.println("Total guesses attempted: " + _shuffle); 
